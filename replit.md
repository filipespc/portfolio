...
- June 23, 2025: Implemented comprehensive markdown formatting support - users can create bold text with `**text**`, italic text with `*text*`, and clickable links with `[text](URL)`. Direct URLs are automatically converted to clickable links. All formatting works in paragraphs and bullet points with proper Sollo red styling for links